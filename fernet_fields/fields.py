...
class EncryptedField(models.Field):
    ...
    def fernet_keys(self):
        if getattr(settings, "FERNET_USE_HKDF", True):
            return [hkdf.derive_fernet_key(k) for k in self.keys]  # type: ignore
        return self.keys

    @cached_property
    def fernet(self):
        if len(self.fernet_keys) == 1:  # type: ignore
            return Fernet(self.fernet_keys[0])  # type: ignore
        return MultiFernet([Fernet(k) for k in self.fernet_keys])  # type: ignore

    def get_internal_type(self):
        return self._internal_type

    def get_db_prep_save(self, value, connection):
        value = super(EncryptedField, self).get_db_prep_save(value, connection)  # type: ignore
...
for name, lookup in models.Field.class_lookups.items():  # type: ignore
